...
public class RestControllerExceptionHandler {
	...
	public ExceptionResponse handleLibraryException(LibraryException ex,WebRequest req) {
		log.error("LibraryException occured = {}",ex.getMessage());
		return new ExceptionResponse(new Date().toString(),HttpStatus.OK.name(),ex.getMessage(),req.getDescription(false));
	}
	
	@ExceptionHandler(FeignException.class)
	public ResponseEntity<ExceptionResponse> handleHttpClientException(FeignException ex,WebRequest webRequest) throws JsonProcessingException {
		log.error("FeignException occured = {}",ex.getMessage());
		ExceptionResponse exceptionResponse = new ObjectMapper().readValue(ex.contentUTF8(),ExceptionResponse.class);
